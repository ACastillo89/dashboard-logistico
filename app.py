...
# ============================================================
st.title("🚚 Dashboard de Gestión Logística y Análisis de Pedidos")
st.markdown("#### Cliente: Indecaucho - Fantape")
...
if top3_clientes_kpi is not None and not top3_clientes_kpi.empty:
    ...
    for i, (col_widget, (nombre, pedidos)) in enumerate(zip(cols_top3, top3_clientes_kpi.items())):
        with col_widget:
            st.markdown(
                f"<div style='line-height:1.5;'>"
                f"<span style='font-size:20px;'>{medallas[i]}</span> "
                f"<span style='font-size:12px; color:#9aa4b8; font-weight:600;'>#{i + 1}</span><br>"
                f"<span style='font-size:13px; font-weight:600;'>{nombre}</span><br>"
                f"<span style='font-size:12px; color:#9aa4b8;'>{int(pedidos)} pedidos</span>"
                f"</div>",
                unsafe_allow_html=True,
            )
else:
    st.caption("Sin datos de clientes para los filtros actuales.")
...
with tab1:
    ...
    fig_dia.update_traces(
        ...
        textfont=dict(color="#F59E0B", size=11, family="Arial Black"),
        line=dict(width=3),
    ...
    if not df_dia.empty:
        rango_x = [
            df_dia["FECHA"].min() - pd.Timedelta(hours=12),
            df_dia["FECHA"].max() + pd.Timedelta(hours=18),
        ]
    else:
        rango_x = None
    ...
    fig_dia.update_layout(
        hovermode="x unified", yaxis_title="Pedidos", height=420,
        margin=dict(r=40, t=30),
    )
    st.plotly_chart(fig_dia, use_container_width=True)

    st.markdown("**Pedidos por Día (tabla compacta, ambos meses)**")
    st.caption("Agrupado por **Fecha de Solicitud**, igual que en 'Comparativo Mes x Día'.")
    if tabla_comparativo is None or tabla_comparativo.empty:
        st.info("Selecciona al menos un mes en la barra lateral (sección 3) para ver esta tabla.")
    else:
        st.dataframe(
            centrar(formatear_numeros(tabla_comparativo)),
    ...
    if tabla_semanal is None or tabla_semanal.empty:
        ...
        # Fila resumen (primera fila): total de pedidos de cada mes, sumando
        # sus semanas. El máximo resaltado en amarillo se calcula SOLO
        # sobre las semanas reales, sin contar esta fila de totales.
        fila_total_sem = tabla_semanal.sum(axis=0, skipna=True)
        fila_total_sem.name = "N° Pedidos Total"
        tabla_semanal_con_total = pd.concat([fila_total_sem.to_frame().T, tabla_semanal])

        def _resaltar_max_por_mes(col):
            maximo = tabla_semanal[col.name].max(skipna=True)
            resultado = []
            for idx, v in zip(tabla_semanal_con_total.index, tabla_semanal_con_total[col.name]):
                if idx == "N° Pedidos Total":
                    resultado.append("background-color: #2b3245; font-weight: 700;")
                elif pd.notna(v) and v == maximo:
                    resultado.append("background-color: #FFEB3B; color: #000000; font-weight: 600;")
                else:
                    resultado.append("")
            return resultado

        st.dataframe(
            centrar(formatear_numeros(tabla_semanal_con_total).style.apply(_resaltar_max_por_mes, axis=0)),
            use_container_width=True,
        )
        ...
        for i, mes in enumerate(tabla_semanal_largo["Mes"].unique()):
            ...
            fig_semanal.add_trace(go.Bar(
                ...
                textfont=dict(color="#000000", size=15, family="Arial Black"),
            ))
        fig_semanal.update_traces(cliponaxis=False)
        fig_semanal.update_layout(
            barmode="group", height=460, xaxis_title="", yaxis_title="Pedidos",
        )
