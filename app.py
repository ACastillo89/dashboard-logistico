...
with tab7:
    ...
    if "CLIENTE" not in df_filtrado.columns:
        ...
        if df_cli.empty:
            ...
            # ---- 2a. Recurrencia por semana, TODOS los meses en paralelo ----
            st.markdown("**Por semana**")
            st.caption("Formato de columna: MM.AA · S# (ej. '08.26 · S1' = Agosto 2026, Semana 1).")

            # Total REAL de pedidos del mes (sin doble conteo: nunique sobre
            # todo el mes, no suma de semanas), mostrado como cajitas en fila
            # (no como columnas que se repartan el ancho completo, para que
            # no se aprieten cuando haya muchos meses — quedan en su tamaño
            # natural, con espacio entre ellas, y hacen scroll horizontal si
            # no caben todas).
            total_por_mes_cli = df_cli.groupby("ANIO_MES")["N_ORDEN"].nunique().sort_index()
            tarjetas_html = "<div style='display:flex; gap:14px; overflow-x:auto; padding:6px 2px 10px;'>"
            for clave, total in total_por_mes_cli.items():
                etiqueta = etiquetas_mes.get(clave, clave)
                tarjetas_html += (
                    "<div style='background:#1c2333; border:1px solid #333c4f; border-radius:10px; "
                    "padding:10px 18px; min-width:130px; flex:0 0 auto;'>"
                    f"<div style='font-size:12px; color:#9aa4b8; text-transform:uppercase; "
                    f"letter-spacing:.03em;'>Total {etiqueta}</div>"
                    f"<div style='font-size:24px; font-weight:700; color:#fff; margin-top:4px;'>{int(total):,}</div>"
                    "</div>"
                )
            tarjetas_html += "</div>"
            ...
            rec_semana = (
                df_cli.groupby(["CLIENTE", "ANIO_MES", "SEMANA_MES"])["N_ORDEN"]
                .nunique()
                .reset_index(name="PEDIDOS")
            )
            ...
            total_real_cliente_sem = df_cli.groupby("CLIENTE")["N_ORDEN"].nunique()
            ...
            fila_resumen_sem["Total"] = df_cli["N_ORDEN"].nunique()
