...
with tab1:
    ...
    with colA:
        ...
        if tabla_semanal is None or tabla_semanal.empty:
            ...
            st.dataframe(
                centrar(tabla_semanal.style.format("{:.0f}", na_rep="—")),
...
with tab3:
    ...
    st.caption(f"Muestra de **Agosto** hasta el mes vigente ({MESES_ES[mes_vigente]}).")
    pivot = comparativo_pivot(df_filtrado, mes_inicio=8, mes_fin=mes_vigente)
    st.dataframe(centrar(pivot.style.format("{:.0f}", na_rep="-")), use_container_width=False)
...
with tab5:
    ...
    st.dataframe(centrar(df_filtrado.drop(columns=["FECHA_DIA"], errors="ignore")), use_container_width=False)
...
with tab6:
    ...
    if col_fecha_solicitud is None:
        ...
        # Tabla arriba (todos los días del mes) y gráfico justo debajo, en el
        # mismo ancho, para que ambos queden alineados y se lean como una
        # sola historia.
        st.dataframe(
            centrar(tabla_comparativo.style.format("{:.0f}", na_rep="—")),
            use_container_width=False,
        )
