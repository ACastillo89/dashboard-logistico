...
with tab7:
    ...
    if "CLIENTE" not in df_filtrado.columns:
        ...
        if not meses_disponibles_cli:
            ...
            # ---- 3. SKU solicitado por cliente, con frecuencia (mes seleccionado) ----
            st.subheader(f"SKU por Cliente — {mes_cli_label}")
            if "SKU" not in df_cli_mes.columns:
                st.info("No se encontró columna **SKU** en el archivo.")
            else:
                col_nombre_sku = "NOMBRE SKU" if "NOMBRE SKU" in df_cli_mes.columns else None
                agg_cols = {"N_ORDEN": "nunique"}
                group_cols = ["CLIENTE", "SKU"] + ([col_nombre_sku] if col_nombre_sku else [])
                tabla_sku_cliente = (
                    df_cli_mes.groupby(group_cols)["N_ORDEN"]
                    .nunique()
                    .reset_index(name="Frecuencia")
                )
                tabla_sku_cliente["Ciudad"] = tabla_sku_cliente["CLIENTE"].map(ciudad_por_cliente)
                columnas_finales = ["CLIENTE", "Ciudad", "SKU"] + ([col_nombre_sku] if col_nombre_sku else []) + ["Frecuencia"]
                tabla_sku_cliente = tabla_sku_cliente[columnas_finales].rename(
                    columns={"CLIENTE": "Cliente", col_nombre_sku: "Nombre SKU"} if col_nombre_sku else {"CLIENTE": "Cliente"}
                )
                tabla_sku_cliente = tabla_sku_cliente.sort_values(["Cliente", "Frecuencia"], ascending=[True, False])

                st.dataframe(
                    centrar(tabla_sku_cliente),
                    use_container_width=True,
                    hide_index=True,
                )
