...
with tab1:
    ...
    with colA:
        ...
        st.caption("Semana del mes vs. mes, según el selector **Mes** de la barra lateral.")
        if tabla_semanal is None or tabla_semanal.empty:
            ...
            st.dataframe(
                centrar(formatear_numeros(tabla_semanal)),
    ...
    # ---- Mes pico vigente y Semana pico vigente, como KPIs destacados ----
    colMesPico, colSemanaPico = st.columns(2)
    with colMesPico:
        fila_mes_vigente = df_mes_top[df_mes_top["ANIO"] == anio_vigente]
        if fila_mes_vigente.empty:
            fila_mes_vigente = df_mes_top.tail(1)
        if not fila_mes_vigente.empty:
            f = fila_mes_vigente.iloc[0]
            colMesPico.metric(
    ...
    with colSemanaPico:
        semanas_pico = todas_semanas[es_pico]
        fila_semana_vigente = semanas_pico[
            (semanas_pico["ANIO"] == anio_vigente) & (semanas_pico["MES_NUM"] == mes_vigente)
        ]
        if fila_semana_vigente.empty:
            fila_semana_vigente = semanas_pico.tail(1)
        if not fila_semana_vigente.empty:
            f = fila_semana_vigente.iloc[0]
    ...
    st.markdown("**Mes con Mayor Volumen por Año**")
    st.dataframe(
        centrar(df_mes_top[["ANIO", "MES_NOMBRE", "PEDIDOS"]].rename(columns={"ANIO": "Año", "MES_NOMBRE": "Mes"})),
        use_container_width=True,
    )

    st.markdown("**Semana Más Fuerte de Cada Mes**")
    ...
    st.dataframe(
        centrar(tabla_semanas_view.style.apply(_resaltar_pico, axis=1)),
        use_container_width=True,
    )
