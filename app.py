...
with tab7:
    ...
    if "CLIENTE" not in df_filtrado.columns:
        ...
        if not meses_disponibles_cli:
            ...
            # clientes en cada semana del mes.
            fila_total_semana = df_cli_mes.groupby("SEMANA_MES")["N_ORDEN"].nunique()
            fila_resumen_sem = pd.Series(index=tabla_rec_semana.columns, dtype=object)
            fila_resumen_sem["Ciudad"] = ""
            for col in cols_sem_num:
                num_semana = int(col.replace("Semana ", ""))
                fila_resumen_sem[col] = fila_total_semana.get(num_semana, 0)
            fila_resumen_sem["Total"] = df_cli_mes["N_ORDEN"].nunique()
