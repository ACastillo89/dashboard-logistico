...
with tab7:
    ...
    if "CLIENTE" not in df_filtrado.columns:
        ...
        if df_cli.empty:
            ...
            paleta_tarjetas = ["#3B82F6", "#10B981", "#F59E0B", "#8B5CF6", "#EC4899", "#06B6D4"]
            tarjetas_html = "<div style='display:flex; gap:14px; overflow-x:auto; padding:6px 2px 10px;'>"
            for i, (clave, total) in enumerate(total_por_mes_cli.items()):
                etiqueta = etiquetas_mes.get(clave, clave)
                color = paleta_tarjetas[i % len(paleta_tarjetas)]
                tarjetas_html += (
                    "<div style='background:#F5F7FA; border:1px solid #E2E6ED; "
                    f"border-top:4px solid {color}; border-radius:10px; "
                    "box-shadow:0 2px 6px rgba(0,0,0,0.08); "
                    "padding:12px 20px; min-width:140px; flex:0 0 auto;'>"
                    f"<div style='font-size:12px; color:{color}; font-weight:700; text-transform:uppercase; "
                    f"letter-spacing:.04em;'>Total {etiqueta}</div>"
                    f"<div style='font-size:26px; font-weight:800; color:#1a1f2b; margin-top:4px;'>{int(total):,}</div>"
                    "</div>"
                )
            tarjetas_html += "</div>"
