...
with tab7:
    ...
    if "CLIENTE" not in df_filtrado.columns:
        ...
        if df_cli.empty:
            ...
            # todo el mes, no suma de semanas), mostrado como tarjetas aparte.
            total_por_mes_cli = df_cli.groupby("ANIO_MES")["N_ORDEN"].nunique().sort_index()
            cols_totales_mes = st.columns(len(total_por_mes_cli))
            for c, (clave, total) in zip(cols_totales_mes, total_por_mes_cli.items()):
                c.metric(f"Total {etiquetas_mes.get(clave, clave)}", f"{int(total):,}")
