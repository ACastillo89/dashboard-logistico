...
# ============================================================
tab1, tab2, tab7, tab6, tab5 = st.tabs([
    "📈 Pedidos por Tiempo",
    "🏙️ Ciudades",
    "👥 Clientes/Ciudad",
...
with tab1:
    ...
    fig_dia = px.line(df_dia, x="FECHA", y="PEDIDOS", markers=True, text="PEDIDOS")
...
with tab7:
    ...
    if "CLIENTE" not in df_filtrado.columns:
        ...
        # ---- Selector local: mes a analizar para el resto de la pestaña ----
        meses_disponibles_cli = sorted(df_cli["ANIO_MES"].unique())
        if not meses_disponibles_cli:
            st.info("No hay datos de clientes para los filtros actuales.")
        else:
            etiquetas_cli = dict(zip(df_cli["ANIO_MES"], df_cli["ANIO_MES_LABEL"]))
            mes_cli_label = st.selectbox(
                "Mes a analizar (para recurrencia y SKU)",
                [etiquetas_cli[k] for k in meses_disponibles_cli],
                index=len(meses_disponibles_cli) - 1,
            )
            mes_cli_key = [k for k in meses_disponibles_cli if etiquetas_cli[k] == mes_cli_label][0]
            df_cli_mes = df_cli[df_cli["ANIO_MES"] == mes_cli_key]
            ...
            # ---- 2a. Recurrencia por semana (del mes seleccionado) ----
            st.subheader(f"Recurrencia de Clientes — {mes_cli_label}")
            st.caption(
                "Cuántas veces solicitó pedidos cada cliente, por semana del mes y en el mes "
                "completo. Los 5 clientes más recurrentes quedan resaltados en amarillo."
            )

            st.markdown("**Por semana**")
            rec_semana = (
                df_cli_mes.groupby(["CLIENTE", "SEMANA_MES"])["N_ORDEN"]
                .nunique()
                .reset_index(name="PEDIDOS")
            )
            tabla_rec_semana = rec_semana.pivot(index="CLIENTE", columns="SEMANA_MES", values="PEDIDOS")
            tabla_rec_semana.columns = [f"Semana {c}" for c in tabla_rec_semana.columns]
            tabla_rec_semana.insert(0, "Ciudad", tabla_rec_semana.index.map(ciudad_por_cliente))
            cols_sem_num = [c for c in tabla_rec_semana.columns if c != "Ciudad"]
            tabla_rec_semana["Total"] = tabla_rec_semana[cols_sem_num].sum(axis=1, skipna=True)
            ...
            # ---- 3. SKU solicitado por cliente, con frecuencia ACUMULADA
            # entre todos los meses activos (no solo el mes seleccionado) ----
            st.subheader("SKU por Cliente")
            st.caption(
                "La frecuencia suma los pedidos de cada SKU en **todos los meses marcados** "
                "en la barra lateral (no solo el mes elegido arriba). El SKU más pedido de "
                "cada cliente queda resaltado en amarillo."
            )
            if "SKU" not in df_cli.columns:
                st.info("No se encontró columna **SKU** en el archivo.")
            else:
                def _limpiar_sku(valor) -> str:
                    """Deja el SKU como texto limpio: sin decimales de Excel
                    (588198.0 -> 588198) y sin seis ceros finales si los trae
                    (ej. 7861234000000 -> 7861234)."""
                    if pd.isna(valor):
                        return None
                    texto = str(valor).strip()
                    if texto.endswith(".0"):
                        texto = texto[:-2]
                    if texto.endswith("000000") and len(texto) > 6:
                        texto = texto[:-6]
                    return texto

                df_sku = df_cli.copy()
                df_sku["SKU_LIMPIO"] = df_sku["SKU"].apply(_limpiar_sku)
                df_sku = df_sku[df_sku["SKU_LIMPIO"].notna()]

                tabla_sku_cliente = (
                    df_sku.groupby(["CLIENTE", "SKU_LIMPIO"])["N_ORDEN"]
                    .nunique()
                    .reset_index(name="Frecuencia")
                    .rename(columns={"CLIENTE": "Cliente", "SKU_LIMPIO": "SKU"})
                )
                tabla_sku_cliente["Ciudad"] = tabla_sku_cliente["Cliente"].map(ciudad_por_cliente)
                tabla_sku_cliente = tabla_sku_cliente[["Cliente", "Ciudad", "SKU", "Frecuencia"]]
                tabla_sku_cliente = tabla_sku_cliente.sort_values(
                    ["Cliente", "Frecuencia"], ascending=[True, False]
                ).reset_index(drop=True)

                # SKU top de cada cliente = el de mayor frecuencia dentro de su grupo
                idx_top_sku = tabla_sku_cliente.groupby("Cliente")["Frecuencia"].idxmax()
                es_top_sku = pd.Series(False, index=tabla_sku_cliente.index)
                es_top_sku.loc[idx_top_sku] = True

                def _resaltar_sku_top(row):
                    if es_top_sku.loc[row.name]:
                        return ["background-color: #FFEB3B; color: #000000; font-weight: 600;"] * len(row)
                    return [""] * len(row)

                st.dataframe(
                    centrar(tabla_sku_cliente.style.apply(_resaltar_sku_top, axis=1)),
                    use_container_width=True,
                    hide_index=True,
                )
...
with tab6:
    ...
    if col_fecha_solicitud is None:
        ...
        fig_dia_comp.update_traces(textposition="outside", cliponaxis=False, textfont_size=10)
