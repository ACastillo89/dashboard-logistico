...
with st.sidebar.expander(
    ...
    if col_ruta_explicita and col_ruta_explicita == mapeo_final.get("ciudad"):
        # La columna de Ciudad ya ES la columna de ruta detectada (caso de
        # este negocio: "RUTA" trae GUAYAQUIL, QUITO, etc.) → no tiene
        # sentido pedirla de nuevo como una columna aparte.
        modo_ruta_default = "Usar la misma columna que Ciudad"
        st.caption(
            f"La columna de Ciudad (**{col_ruta_explicita}**) ya es la ruta/zona, "
            "así que se usa la misma."
        )
