...
if col_ruta_explicita and col_ruta_explicita == mapeo_final.get("ciudad"):
    # La columna de Ciudad ya ES la columna de ruta (caso de este negocio:
    # "RUTA" trae GUAYAQUIL, QUITO, etc.) → se reutiliza, sin pedirla aparte.
    mapeo_final["ruta_modo"] = "ciudad"
elif col_ruta_explicita:
    mapeo_final["ruta_modo"] = "columna"
    mapeo_final["ruta_columna"] = col_ruta_explicita
elif col_fuente_ruta:
    mapeo_final["ruta_modo"] = "derivada"
    mapeo_final["ruta_fuente"] = col_fuente_ruta
    mapeo_final["ruta_derivacion_modo"] = "segmento_final"
else:
    mapeo_final["ruta_modo"] = "ciudad"
...
# Nota: no hay selector de Ruta — el análisis siempre considera TODAS las
# rutas recorridas, tal como se requiere.
...
with tab2:
    ...
    with colB:
        ...
        fig_ruta.update_traces(textposition="outside", cliponaxis=False)
        fig_ruta.update_layout(
            yaxis_title="", xaxis_title="Pedidos",
            margin=dict(r=40),
            height=max(500, 32 * len(rank_ruta)),
            bargap=0.1,
        )
        fig_ruta.update_xaxes(range=[0, rank_ruta["PEDIDOS"].max() * 1.15])
